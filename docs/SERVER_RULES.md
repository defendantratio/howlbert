...
* failing to warn when required: a warning, then a temporary mute on repeat.
...
## character boundaries must be documented

* every player may list hard limits (no eye trauma, no humiliation, no permanent death, etc.) on their character sheet or `/profile`.
* violating another player's stated boundary is a serious offense; expect a mute or kick. ask first if you're not sure where a line is.

## credit all creative work; no art theft
...
* using an alt account to join the server? tell a staff member and link it to your main.
* ban evasion via an alt account is an instant permanent ban on every account involved.
* using an alt to dodge a mute, kick, or role restriction escalates further.
...
* these rules cover the most common scenarios, but they can't predict everything.
* staff can act on behavior that breaks the spirit of these rules even if it isn't explicitly listed. "i didn't see it in the rules" isn't a valid defense.
* not sure if something's okay? ask a mod first.

this is a space for creativity and respect. treat others how you want to be treated, and help keep it safe for everyone.
