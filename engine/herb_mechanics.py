...
def build_usage_hint(herb_key: str, meta: dict | None = None) -> str:
    """Single-line mechanical hint for compendium pages, on top of the effect/treats
    lines already shown; empty when there's nothing extra to add."""
    meta = meta or HERBS.get(herb_key, {})
    if meta.get("poison") or meta.get("rarity") == "restricted":
        return STATIC_HINTS.get(herb_key, "restricted; medic knowledge only.")

    if herb_key in STATIC_HINTS:
        return STATIC_HINTS[herb_key]
    ...
    if special and special in SPECIAL_MECHANICS:
        return SPECIAL_MECHANICS[special]
