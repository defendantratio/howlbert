...
from engine.proxy import (
    ...
    parse_tupperbox_export,
    sanitize_webhook_name,
)
...
_MAX_PROXY_ATTACH = 8 * 1024 * 1024  # 8 MB; skip larger files rather than fail the post
...
class Proxy(commands.Cog):
    ...
    proxy = app_commands.Group(
        name="proxy",
        description="Speak in-character as your wolves (tupperbox-style) and import from Tupperbox.",
    )
    ...
    @app_commands.describe(
        tag="Tag template using 'text' as the placeholder, e.g. H:text or [text]",
        own_wolf="Which of your wolves (defaults to your active wolf)",
    )
    @app_commands.autocomplete(own_wolf=_own_wolf_autocomplete)
    async def set_tag(
        self, interaction: discord.Interaction, tag: str, own_wolf: str | None = None
    ):
        ...
        if not prefix and not suffix:
            await interaction.response.send_message(
                embed=howlbert_embed(
                    "Bad Tag",
                    "Give a tag like `H:text`, `[text]`, or `text-h` (use `text` as the placeholder).",
                    color=ERROR_COLOR,
                ),
                ephemeral=reply_ephemeral(),
            )
        ...
        body = (
            f"**{wolf['wolf_name']}** will now speak when you type `{example}`.\n"
            "Set an avatar with `/proxy avatar`. Bot needs **Manage Webhooks** + **Manage Messages** here."
        )
        await interaction.response.send_message(
            embed=howlbert_embed("Proxy Tag Set", body, color=SUCCESS_COLOR),
            ephemeral=reply_ephemeral(),
        )

    @proxy.command(name="avatar", description="Set the avatar image a wolf proxies with.")
    @app_commands.describe(
        image="Upload an image file (opens the crop editor)",
        url="Or paste a direct image URL (opens the crop editor)",
        clear="Remove this wolf's proxy avatar",
        own_wolf="Which of your wolves (defaults to your active wolf)",
    )
    @app_commands.autocomplete(own_wolf=_own_wolf_autocomplete)
    async def set_avatar(
        ...
        if clear:
            if image or url:
                await interaction.response.send_message(
                    embed=howlbert_embed(
                        "Pick One",
                        "Use `clear:true` by itself, or provide an image — not both.",
                        color=ERROR_COLOR,
                    ),
                    ephemeral=reply_ephemeral(),
                )
            ...
            await interaction.response.send_message(
                embed=howlbert_embed(f"{wolf['wolf_name']}", "Proxy avatar cleared.", color=SUCCESS_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        if image and url:
            await interaction.response.send_message(
                embed=howlbert_embed(
                    "Pick One",
                    "Attach an **image** or give a **url** — not both.",
                    color=ERROR_COLOR,
                ),
                ephemeral=reply_ephemeral(),
            )
        ...
        if not image and not url:
            await interaction.response.send_message(
                embed=howlbert_embed(
                    "No Image",
                    "Attach an **image** to this command, paste a direct **url**, or use `clear:true`.",
                    color=ERROR_COLOR,
                ),
                ephemeral=reply_ephemeral(),
            )
        ...
        try:
            if image:
                ...
                err_msg = validate_avatar_upload(
                    data, content_type=image.content_type, filename=image.filename
                )
            else:
                if not url.lower().startswith(("http://", "https://")):
                    await interaction.followup.send(
                        embed=howlbert_embed("Bad URL", "Give a direct `http(s)` image link.", color=ERROR_COLOR),
                        ephemeral=reply_ephemeral(),
                    )
                    return
            ...
            await interaction.followup.send(
                embed=howlbert_embed("Download Failed", "Could not fetch that image URL.", color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        except Exception:
            ...
            await interaction.followup.send(
                embed=howlbert_embed("Read Failed", "Could not read that image.", color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        ...
        if err_msg:
            await interaction.followup.send(
                embed=howlbert_embed("Bad Image", err_msg, color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return

        if not interaction.guild:
            await interaction.followup.send("Use this in a server.", ephemeral=reply_ephemeral())
            return
    ...
    async def clear_tag(self, interaction: discord.Interaction, own_wolf: str | None = None):
        ...
        await interaction.response.send_message(
            embed=howlbert_embed(f"{wolf['wolf_name']}", "Proxy tag removed.", color=SUCCESS_COLOR),
            ephemeral=reply_ephemeral(),
        )

    @proxy.command(
        name="autoproxy",
        description="Proxy ALL your untagged messages as one wolf (or turn it off).",
    )
    @app_commands.describe(
        mode="on = proxy as the chosen wolf; off = stop autoproxy",
        own_wolf="Wolf to autoproxy as (defaults to your active wolf)",
    )
    ...
    async def autoproxy(
        ...
        if mode.value == "off":
            db.set_autoproxy_wolf(interaction.user.id, None)
            await interaction.response.send_message(
                embed=howlbert_embed("Autoproxy Off", "Untagged messages stay as you.", color=SUCCESS_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        wolf, err = _resolve_wolf(interaction, own_wolf)
        ...
        await interaction.response.send_message(
            embed=howlbert_embed(
                "Autoproxy On",
                f"Untagged messages now post as **{wolf['wolf_name']}**. "
                "Start a message with `\\` to send as yourself once.",
                color=SUCCESS_COLOR,
            ),
            ephemeral=reply_ephemeral(),
        )
    ...
    @app_commands.describe(
        file="Your Tupperbox export JSON (DM Tupperbox `tul!export`) or PluralKit export",
        link_to_wolves="Match imported names to your existing wolves (default: yes)",
    )
    async def import_proxies(
        self,
        interaction: discord.Interaction,
        file: discord.Attachment,
        link_to_wolves: bool = True,
    ):
        await interaction.response.defer(ephemeral=reply_ephemeral())
        if not (file.filename.lower().endswith(".json") or "json" in (file.content_type or "")):
            await interaction.followup.send(
                embed=howlbert_embed("Wrong File", "Attach the `.json` export file.", color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        try:
            ...
            await interaction.followup.send(
                embed=howlbert_embed("Import Failed", str(exc), color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        except Exception:
            ...
            await interaction.followup.send(
                embed=howlbert_embed("Import Failed", "Could not read that export file.", color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        ...
        if not entries:
            await interaction.followup.send(
                embed=howlbert_embed("Nothing to Import", "No proxies found in that file.", color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        ...
        if unmatched:
            body.append(
                f"\n**{len(unmatched)}** export name(s) had no matching wolf "
                f"(rename a wolf to match, then re-import): {', '.join(unmatched[:15])}"
            )
        await interaction.followup.send(
            embed=howlbert_embed("Tupperbox Import", "\n".join(body), color=SUCCESS_COLOR),
            ephemeral=reply_ephemeral(),
        )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if content.startswith("\\"):
            return  # explicit escape; send as yourself
    ...
    async def _send_as_wolf(self, message: discord.Message, wolf, content: str) -> None:
        ...
        try:
            ...
            if footer_bits:
                kwargs["embeds"] = [
                    discord.Embed(color=discord.Color.dark_grey()).set_footer(
