...
async def post_collab_party_call(interaction: discord.Interaction, bot: commands.Bot, *, patrol_kind: str='survey') -> None:
    ...
    err = validate_start_collab_patrol(user, guild_id=interaction.guild.id, day=day, kind=patrol_kind)
    if err:
        if war:
        ...
        await interaction.response.send_message(embed=embed, ephemeral=reply_ephemeral())
        return
    patrol_id = db.create_collab_patrol(guild_id=interaction.guild.id, channel_id=interaction.channel_id, leader_wolf_id=user['id'], pack_id=user['pack_id'], day_number=day, patrol_kind=patrol_kind)
    db.add_collab_patrol_member(patrol_id, wolf_id=user['id'], wolf_name=user['wolf_name'], discord_id=user['discord_id'])
    ...
    await interaction.followup.send(embed=howlbert_embed(done_title, done_body, color=SUCCESS_COLOR), ephemeral=reply_ephemeral())
...
class CollabPatrolCog(commands.Cog):
    ...
    async def apply_join(interaction: discord.Interaction, patrol_id: int, wolf) -> None:
        ...
        if not patrol or patrol['status'] != 'open':
            await interaction.response.send_message(embed=howlbert_embed('Patrol Closed', 'This pack patrol is no longer open.', color=ERROR_COLOR), ephemeral=reply_ephemeral())
            return
        day = db.get_world(patrol['guild_id'])['day_number']
        err = validate_join_collab_patrol(wolf, patrol, day)
        if err:
            await interaction.response.send_message(embed=howlbert_embed("Can't Join", err, color=ERROR_COLOR), ephemeral=reply_ephemeral())
            return
        await interaction.response.defer(thinking=False)
        db.add_collab_patrol_member(patrol_id, wolf_id=wolf['id'], wolf_name=wolf['wolf_name'], discord_id=wolf['discord_id'])
        embed = build_collab_patrol_embed(patrol_id)
        ...
        if channel and patrol['message_id']:
            try:
                ...
                pass
        await interaction.followup.send(embed=howlbert_embed('Joined', f"**{wolf['wolf_name']}** joins the patrol.", color=SUCCESS_COLOR), ephemeral=reply_ephemeral())
    ...
    async def handle_join(interaction: discord.Interaction, patrol_id: int) -> None:
        ...
        patrol = db.get_collab_patrol(patrol_id)
        if not patrol:
            await interaction.response.send_message(player_message('Patrol not found.'), ephemeral=reply_ephemeral())
            return
        day = db.get_world(patrol['guild_id'])['day_number']
        eligible = wolves_eligible_to_join_patrol(interaction.user.id, patrol_id, day)
        if not eligible:
            await interaction.response.send_message(embed=howlbert_embed("Can't Join", 'No eligible scout on your account (wrong pack, already surveyed, or already joined).', color=ERROR_COLOR), ephemeral=reply_ephemeral())
            return
        if len(eligible) == 1:
            await CollabPatrolCog.apply_join(interaction, patrol_id, eligible[0])
            return
        view = PatrolWolfSelectView(patrol_id, eligible)
        await interaction.response.send_message(embed=howlbert_embed('Choose Your Wolf', 'Which scout joins the patrol?', color=SUCCESS_COLOR), view=view, ephemeral=reply_ephemeral())
    ...
    async def handle_set_out(interaction: discord.Interaction, patrol_id: int) -> None:
        ...
        if not user or user['id'] != patrol['leader_wolf_id']:
            await interaction.response.send_message(embed=howlbert_embed('Caller Only', 'Only the scout who called this patrol can set out.', color=ERROR_COLOR), ephemeral=reply_ephemeral())
            return
        members = db.get_collab_patrol_members(patrol_id)
        if len(members) < COLLAB_PATROL_MIN_WOLVES:
            await interaction.response.send_message(embed=howlbert_embed('Too Few Scouts', f'Need at least **{COLLAB_PATROL_MIN_WOLVES}** scouts before setting out.', color=ERROR_COLOR), ephemeral=reply_ephemeral())
            return
        await interaction.response.defer(thinking=False)
        ...
        if enc_id:
            ...
            await interaction.followup.send(embed=howlbert_embed('Ambush!', 'The party fights together below (+1 attack per ally, max +3).', color=SUCCESS_COLOR), ephemeral=reply_ephemeral())
            return
        if channel and patrol['message_id']:
            try:
                msg = await channel.fetch_message(patrol['message_id'])
        ...
        await interaction.followup.send(embed=howlbert_embed('Away!', done, color=SUCCESS_COLOR), ephemeral=reply_ephemeral())

    @staticmethod
    async def handle_cancel(interaction: discord.Interaction, patrol_id: int) -> None:
        patrol = db.get_collab_patrol(patrol_id)
        if not patrol or patrol['status'] != 'open':
            await interaction.response.send_message(player_message('This party is already closed.'), ephemeral=reply_ephemeral())
            return
        user = db.get_user(interaction.user.id)
        if not user or user['id'] != patrol['leader_wolf_id']:
            await interaction.response.send_message(embed=howlbert_embed('Caller Only', 'Only the scout who called this party can cancel it.', color=ERROR_COLOR), ephemeral=reply_ephemeral())
            return
        ...
        await interaction.response.send_message(embed=howlbert_embed('Cancelled', f'The pack {label} was called off.', color=ERROR_COLOR), ephemeral=reply_ephemeral())
