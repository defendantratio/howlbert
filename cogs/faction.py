...
class Faction(commands.Cog):
    ...
    async def observe(self, interaction: discord.Interaction, faction: str):
        ...
        last = int(user["last_faction_action_day"]) if "last_faction_action_day" in user.keys() else 0
        if last >= day:
            await interaction.response.send_message(embed=howlbert_embed("Already Acted", "You've already taken a faction action this sunrise. Resets at next rollover.", color=ERROR_COLOR), ephemeral=reply_ephemeral())
            return
        gp = user["great_pack"] if "great_pack" in user.keys() else None
        if not gp:
    ...
    async def approach(self, interaction: discord.Interaction, faction: str):
        ...
        last = int(user["last_faction_action_day"]) if "last_faction_action_day" in user.keys() else 0
        if last >= day:
            await interaction.response.send_message(embed=howlbert_embed("Already Acted", "You've already taken a faction action this sunrise.", color=ERROR_COLOR), ephemeral=reply_ephemeral())
            return
        gp = user["great_pack"] if "great_pack" in user.keys() else None
        if not gp:
            await interaction.response.send_message(embed=howlbert_embed("No Pack", "Faction actions require a Great Pack affiliation.", color=ERROR_COLOR), ephemeral=reply_ephemeral())
