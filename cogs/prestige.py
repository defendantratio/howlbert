...
from engine.prestige import (
    bone_bonus_pct,
    format_requirement_progress,
    format_retirement_breakdown,
    get_tier_info,
...
class Prestige(commands.Cog):
    ...
    async def _prestige_require(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name="Legacy",
            value=format_requirement_progress(account["legacy_score"], nxt["legacy_req"]),
            inline=True,
        )
        embed.add_field(
            name="Quests",
            value=format_requirement_progress(account["total_quests"], nxt["quests_req"]),
            inline=True,
        )
        embed.add_field(
            name="Hunts",
            value=format_requirement_progress(account["total_hunts"], nxt["hunts_req"]),
            inline=True,
        )
        embed.add_field(
            name="Retirements",
            value=format_requirement_progress(account["total_retirements"], nxt["retirements_req"]),
            inline=True,
        )
        embed.add_field(
            name="Unlocks",
    ...
    async def _legacy(self, interaction: discord.Interaction):
        ...
        if user:
            embed.add_field(
                name="Retire This Wolf",
                value=format_retirement_breakdown(
                    int(user["standing"]), int(user["bones"]), quests_done
                ),
                inline=False,
            )
    ...
    async def _retire(self, interaction: discord.Interaction):
        ...
        if not result:
            await interaction.response.send_message("Retirement failed.", ephemeral=reply_ephemeral())
            return
