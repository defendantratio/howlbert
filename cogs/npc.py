...
class Npc(commands.Cog):
        ...
        if not rows:
            await interaction.response.send_message(
                embed=howlbert_embed(
                    "No NPCs",
                    "Admins can add one with `/npc add`.",
                    color=ERROR_COLOR,
                ),
    ...
    @app_commands.describe(
        ...
        avatar="Image URL for their portrait",
        tag="Optional proxy-style tag prefix (e.g. OldCrow:)",
    )
    async def add_npc(
        self,
        ...
        if not interaction.guild or not is_howlbert_admin(interaction):
            await interaction.response.send_message(
                embed=howlbert_embed("Admin Only", "Only admins can add NPCs.", color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        prefix = suffix = None
        ...
        if not npc_id:
            await interaction.response.send_message(
                embed=howlbert_embed(
                    "Failed",
                    "That name may already exist or was invalid.",
                    color=ERROR_COLOR,
                ),
                ephemeral=reply_ephemeral(),
            )
        ...
        await interaction.response.send_message(
            embed=howlbert_embed(
                "NPC Added",
                f"**{name.strip()}** is in the registry. Use `/npc say`.",
                color=SUCCESS_COLOR,
            ),
            ephemeral=reply_ephemeral(),
        )
    ...
    async def remove_npc(self, interaction: discord.Interaction, name: str):
        if not interaction.guild or not is_howlbert_admin(interaction):
            await interaction.response.send_message(
                embed=howlbert_embed("Admin Only", "Only admins can remove NPCs.", color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        if db.delete_server_npc(interaction.guild.id, name):
            await interaction.response.send_message(
                embed=howlbert_embed("Removed", f"**{name.strip()}** left the registry.", color=SUCCESS_COLOR),
                ephemeral=reply_ephemeral(),
            )
        else:
            await interaction.response.send_message(
                embed=howlbert_embed("Not Found", "No NPC by that name.", color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
    ...
    async def say_npc(self, interaction: discord.Interaction, name: str, line: str):
        ...
        if not npc:
            await interaction.response.send_message(
                embed=howlbert_embed("Unknown NPC", "Use `/npc list`.", color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        text = line.strip()
        if not text:
            await interaction.response.send_message(
                embed=howlbert_embed("Empty", "Give them a line.", color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        if len(text) > 500:
