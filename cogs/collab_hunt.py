...
from engine.collab_hunt import (
    build_collab_hunt_embed,
    try_set_out_collab_hunt,
    validate_join_collab_hunt,
    validate_start_collab_hunt,
...
class CollabWolfSelect(discord.ui.Select):
    def __init__(self, hunt_id: int, wolves: list):
        ...
        super().__init__(
            placeholder="Which wolf joins the hunt?",
            min_values=1,
            max_values=1,
            options=options,
        )
...
def make_collab_hunt_view(hunt_id: int) -> discord.ui.View:
    ...
    join_btn = discord.ui.Button(
        ...
        emoji="🐺",
        custom_id=f"howlbert_collab:{hunt_id}:join",
    )
    ...
    go_btn = discord.ui.Button(
        ...
        emoji="🏹",
        custom_id=f"howlbert_collab:{hunt_id}:go",
    )
    ...
    cancel_btn = discord.ui.Button(
        ...
        custom_id=f"howlbert_collab:{hunt_id}:cancel",
    )
...
async def post_collab_hunt_call(
...
class CollabHuntCog(commands.Cog):
    ...
    async def handle_join(interaction: discord.Interaction, hunt_id: int) -> None:
        ...
        await interaction.response.send_message(
            embed=howlbert_embed(
                "Choose Your Wolf",
                "Which character joins the pack hunt?",
                color=SUCCESS_COLOR,
            ),
            view=view,
            ephemeral=True,
