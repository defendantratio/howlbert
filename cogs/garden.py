...
async def _need_user_guild(interaction: discord.Interaction):
    ...
    if not interaction.guild:
        await interaction.response.send_message(
            embed=howlbert_embed("Server Only", "Use the garden in a server.", color=ERROR_COLOR),
            ephemeral=reply_ephemeral(),
        )
        return None, None
...
async def _need_pack_garden(interaction: discord.Interaction):
    ...
    if not pack_id:
        await interaction.response.send_message(
            embed=howlbert_embed(
                "No Pack",
                "The herb garden is a **pack** plot. Join a pack first (`/pack action:join`).",
                color=ERROR_COLOR,
            ),
            ephemeral=reply_ephemeral(),
        )
    ...
    if not pack:
        await interaction.response.send_message(
            embed=howlbert_embed("No Pack", "Your pack record is missing.", color=ERROR_COLOR),
            ephemeral=reply_ephemeral(),
        )
        return None, None, None
...
class Garden(commands.Cog):
    garden = app_commands.Group(
        name="garden",
        description="Pack herb garden; sow seeds, tend plots, and harvest.",
    )
    ...
    async def plots(self, interaction: discord.Interaction):
        ...
        if not plantings:
            await interaction.response.send_message(
                embed=howlbert_embed(
                    "Empty Garden",
                    f"**{pack['name']}** has no herb plots yet. Forage or `/garden buy` for seeds, "
                    "then `/garden plant`.",
                    color=SUCCESS_COLOR,
                ),
                ephemeral=reply_ephemeral(),
        ...
        for p in plantings:
            ...
            if res.dead:
                ...
                lines.append(
                    ...
                    f"· HP {res.health} · {res.note} · planted by {who}"
                )
        body = "\n".join(lines)
        embed = howlbert_embed(f"{pack['name']} Herb Garden", body, color=SUCCESS_COLOR)
        embed.set_footer(
            text=f"Season: {season} · /garden tend · /garden harvest · "
            f"{len(plantings)}/{GARDEN_MAX_PLOTS} plots"
        )
    ...
    async def seeds(self, interaction: discord.Interaction):
        ...
        if not rows:
            body = (
                ...
                "a mature plant (keeps seeds for next time). Seeds are yours; plots belong to the pack."
            )
        else:
            body = "\n".join(
    ...
    async def plant(self, interaction: discord.Interaction, herb: str):
        ...
        if db.count_pack_herb_plantings(pack["id"]) >= GARDEN_MAX_PLOTS:
            await interaction.response.send_message(
                embed=howlbert_embed(
                    "Garden Full",
                    f"**{pack['name']}** can tend **{GARDEN_MAX_PLOTS}** plots at once. Harvest or clear one first.",
                    color=ERROR_COLOR,
                ),
                ephemeral=reply_ephemeral(),
            )
        ...
        embed = howlbert_embed(
            "Seed Sown",
            f"**{user['wolf_name']}** presses a **{_herb_name(herb)}** seed into "
            f"**{pack['name']}**'s plot `#{pid}`.\n\n"
            f"{growing_blurb(herb)}{warn}\n\n"
            "_Sowing isn't tending — water it with `/garden tend`._",
            color=SUCCESS_COLOR,
        )
    ...
    async def tend(self, interaction: discord.Interaction):
        ...
        if not living:
            await interaction.response.send_message(
                embed=howlbert_embed(
                    "Nothing to Tend",
                    f"**{pack['name']}** has no living plots. Sow a seed with `/garden plant`.",
                    color=ERROR_COLOR,
                ),
                ephemeral=reply_ephemeral(),
            )
        ...
        if db.pack_garden_tended_today(pack["id"], day):
            await interaction.response.send_message(
                embed=howlbert_embed(
                    "Already Tended",
                    f"**{pack['name']}**'s garden was already watered this sunrise. Come back next sunrise.",
                    color=ERROR_COLOR,
                ),
                ephemeral=reply_ephemeral(),
            )
        ...
        body = (
            f"**{user['wolf_name']}** waters and weeds **{pack['name']}**'s garden "
            f"(**{tended}** plot(s))."
        )
        await interaction.response.send_message(
            embed=howlbert_embed("Garden Tended", body, color=SUCCESS_COLOR)
        )
    ...
    async def harvest(self, interaction: discord.Interaction, plot: int | None = None):
        ...
        if plot is not None:
            ...
            if not plantings:
                await interaction.response.send_message(
                    embed=howlbert_embed(
                        "No Such Plot",
                        f"Plot `#{plot}` isn't in **{pack['name']}**'s garden.",
                        color=ERROR_COLOR,
                    ),
                    ephemeral=reply_ephemeral(),
                )
        ...
        for p in plantings:
            ...
            harvested += 1
            lines.append(
                f"🌼 **{_herb_name(p['herb_key'])}** — {count} fresh stack(s) + {seeds_back} seed(s) to your pouch."
            )
    ...
    async def clear(self, interaction: discord.Interaction, plot: int):
        ...
        await interaction.response.send_message(
            embed=howlbert_embed(
                "Plot Cleared",
                f"**{user['wolf_name']}** pulls up the **{_herb_name(target['herb_key'])}** in plot `#{plot}`.",
                color=SUCCESS_COLOR,
            )
        )
