"""Grow-your-own herbs: sow seeds, tend plots, and harvest at the den."""
...
from config import (
    GARDEN_HARVEST_SEED_MAX,
    GARDEN_HARVEST_SEED_MIN,
    GARDEN_MAX_PLOTS,
    GARDEN_SEED_BONE_COST,
    GARDEN_TEND_HEALTH_RESTORE,
)
...
def _tick(planting, season: str, day: int):
    ...
    result, updates = evaluate_growth(
...
async def _need_user_guild(interaction: discord.Interaction):
    ...
    if not interaction.guild:
        await interaction.response.send_message(
            embed=howlbert_embed("Server Only", "Tend your garden in a server.", color=ERROR_COLOR),
            ephemeral=reply_ephemeral(),
        )
        return None, None
...
async def _cultivable_autocomplete(interaction, current: str):
    ...
    for key in cultivable_herbs():
        ...
        if current and current.lower() not in name.lower() and current.lower() not in key:
            continue
        out.append(app_commands.Choice(name=f"{name} ({key})"[:100], value=key))
    return out[:25]


class Garden(commands.Cog):
    garden = app_commands.Group(
        name="garden",
        description="Grow your own herbs; sow seeds, tend plots, and harvest.",
    )

    def __init__(self, bot: commands.Bot):
        self.bot = bot

    @garden.command(name="plots", description="View your herb garden and its growth.")
    async def plots(self, interaction: discord.Interaction):
        ...
        plantings = db.get_herb_plantings(user["id"])
        if not plantings:
            await interaction.response.send_message(
                embed=howlbert_embed(
                    "Empty Garden",
                    "No plots yet. Get seeds with `/garden buy` or by foraging, then "
                    "sow with `/garden plant`.",
                    color=SUCCESS_COLOR,
                ),
                ephemeral=reply_ephemeral(),
            )
        ...
        for p in plantings:
            ...
            if res.dead:
                lines.append(f"{icon} `#{p['id']}` **{_herb_name(p['herb_key'])}** — dead ({res.note})")
            elif res.ready:
                lines.append(f"{icon} `#{p['id']}` **{_herb_name(p['herb_key'])}** — ready to harvest!")
            else:
                lines.append(
                    f"{icon} `#{p['id']}` **{_herb_name(p['herb_key'])}** — {res.progress_pct}% "
                    f"· HP {res.health} · {res.note}"
                )
        body = "\n".join(lines)
        embed = howlbert_embed(f"{user['wolf_name']}'s Herb Garden", body, color=SUCCESS_COLOR)
    ...
    async def seeds(self, interaction: discord.Interaction):
        ...
        if not rows:
            body = (
                "Your seed pouch is empty.\n\n"
                "Get seeds by **foraging** (`/field action:forage`), buying a packet "
                f"(`/garden buy`, **{GARDEN_SEED_BONE_COST} bones**), or **harvesting** "
                "a mature plant (keeps seeds for next time)."
            )
        else:
            body = "\n".join(
                f"**{_herb_name(r['herb_key'])}** ×{r['qty']} — {growing_blurb(r['herb_key'])}"
                for r in rows
            )
    ...
    @garden.command(name="plant", description="Sow a seed from your pouch into a new plot.")
    ...
    async def plant(self, interaction: discord.Interaction, herb: str):
        ...
        if not can_cultivate(herb):
            await interaction.response.send_message(
                embed=howlbert_embed("Can't Sow That", "That herb can't be grown in a garden.", color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        if db.count_herb_plantings(user["id"]) >= GARDEN_MAX_PLOTS:
            await interaction.response.send_message(
                embed=howlbert_embed(
                    "Garden Full",
                    f"You can tend **{GARDEN_MAX_PLOTS}** plots at once. Harvest or clear one first.",
                    color=ERROR_COLOR,
                ),
                ephemeral=reply_ephemeral(),
            )
        ...
        if db.get_herb_seed_qty(user["id"], herb) <= 0:
            await interaction.response.send_message(
                embed=howlbert_embed(
                    "No Seeds",
                    f"You have no **{_herb_name(herb)}** seeds. Try `/garden buy` or forage for some.",
                    color=ERROR_COLOR,
                ),
                ephemeral=reply_ephemeral(),
            )
        ...
        pid = db.add_herb_planting(user["id"], herb, guild_id=interaction.guild.id, day=day, season=season)
        profile = growing_profile(herb)
        warn = ""
        if season not in profile.seasons:
            warn = (
                "\n\n_⚠ Off-season sowing; it'll grow slowly_"
                if profile.hardy
                else "\n\n_⚠ Wrong season; it may struggle or die_"
            )
        embed = howlbert_embed(
            "Seed Sown",
            f"You press a **{_herb_name(herb)}** seed into plot `#{pid}`.\n\n"
            f"{growing_blurb(herb)}{warn}",
            color=SUCCESS_COLOR,
        )
        embed.set_footer(text="/garden tend each sunrise · /garden plots")
        await interaction.response.send_message(embed=embed)

    @garden.command(name="tend", description="Water and weed all your plots (once per sunrise).")
    async def tend(self, interaction: discord.Interaction):
        ...
        if not living:
            await interaction.response.send_message(
                embed=howlbert_embed("Nothing to Tend", "No living plots. Sow a seed with `/garden plant`.", color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        ...
        tended, already = 0, 0
        for p in living:
            _tick(p, season, day)  # apply pending growth/penalties first
            fresh = db.get_herb_planting(p["id"])
            if not fresh or fresh["dead"]:
                continue
            if int(fresh["last_tended_day"]) >= day:
                already += 1
                continue
        ...
        if tended == 0 and already:
            body = "You've already watered the garden this sunrise. Come back next sunrise."
            color = ERROR_COLOR
        else:
            body = f"You water and weed **{tended}** plot(s)."
            if already:
                body += f" ({already} already tended today.)"
            color = SUCCESS_COLOR
        await interaction.response.send_message(
            embed=howlbert_embed("Garden Tended", body, color=color)
        )
    ...
    async def harvest(self, interaction: discord.Interaction, plot: int | None = None):
        ...
        if plot is not None:
            ...
            if not plantings:
                await interaction.response.send_message(
                    embed=howlbert_embed("No Such Plot", f"Plot `#{plot}` isn't in your garden.", color=ERROR_COLOR),
                    ephemeral=reply_ephemeral(),
                )
                return
        ...
        for p in plantings:
            ...
            for _ in range(count):
                grant_fresh_herb(
                    user["id"], herb_key=p["herb_key"], guild_id=interaction.guild.id, day=day, user=user
                )
            seeds_back = max(
                GARDEN_HARVEST_SEED_MIN,
                GARDEN_HARVEST_SEED_MAX if res.health >= 80 else GARDEN_HARVEST_SEED_MIN,
            )
            db.add_herb_seeds(user["id"], p["herb_key"], seeds_back)
            db.remove_herb_planting(p["id"])
            harvested += 1
            lines.append(
                f"🌼 **{_herb_name(p['herb_key'])}** — {count} fresh stack(s) + {seeds_back} seed(s)."
            )
        ...
        if not lines:
            await interaction.response.send_message(
                embed=howlbert_embed(
                    "Nothing Ready",
                    "No plots are mature yet. Check `/garden plots` and keep tending.",
                    color=ERROR_COLOR,
                ),
                ephemeral=reply_ephemeral(),
            )
    ...
    async def clear(self, interaction: discord.Interaction, plot: int):
        ...
        if not target or target["wolf_id"] != user["id"]:
            await interaction.response.send_message(
                embed=howlbert_embed("No Such Plot", f"Plot `#{plot}` isn't in your garden.", color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
        ...
        await interaction.response.send_message(
            embed=howlbert_embed(
                "Plot Cleared",
                f"You pull up the **{_herb_name(target['herb_key'])}** in plot `#{plot}`.",
                color=SUCCESS_COLOR,
            )
        )
    ...
    async def buy(self, interaction: discord.Interaction, herb: str):
        ...
        if not can_cultivate(herb):
            await interaction.response.send_message(
                embed=howlbert_embed("Unavailable", "Those seeds aren't sold here.", color=ERROR_COLOR),
                ephemeral=reply_ephemeral(),
            )
            return
        if not db.deduct_bones(interaction.user.id, GARDEN_SEED_BONE_COST):
            await interaction.response.send_message(
                embed=howlbert_embed(
                    "Not Enough Bones",
                    f"Seed packets cost **{GARDEN_SEED_BONE_COST} bones**.",
                    color=ERROR_COLOR,
                ),
                ephemeral=reply_ephemeral(),
            )
        ...
        embed = howlbert_embed(
            "Seeds Bought",
            f"You trade **{GARDEN_SEED_BONE_COST} bones** for a packet of "
            f"**{_herb_name(herb)}** seeds.\n\n{growing_blurb(herb)}",
            color=SUCCESS_COLOR,
        )
    ...
    async def guide(self, interaction: discord.Interaction, herb: str | None = None):
        if herb:
            ...
            if not can_cultivate(herb):
                await interaction.response.send_message(
                    embed=howlbert_embed("Not Cultivable", "That herb is foraged from the wild, not grown.", color=ERROR_COLOR),
                    ephemeral=reply_ephemeral(),
                )
                return
            meta = HERBS.get(herb, {})
