...
✦ **few hard blocks; energy, not cooldowns**: instead of "come back next sunrise" walls, every field/social/den action spends **energy** (shown on `/vitals action:condition`). energy never refuses a command; acting at **0** just adds exhaustion and costs mood, on top of the exhaustion/pain-exhaustion layer rather than replacing it. energy restores each **sunrise** sleep, from `/vitals action:rest`, and via a slow **real-time drip** while a wolf is idle during the day, all scaled by **hunger** (a starving wolf recovers less). every repeatable field action; hunt, forage, verge, explore, survey, trail, track, gossip, and the rest; is throttled purely by energy now (no per-sunrise cap, no climbing dc, no shrinking payout). the only twist is that specialists tire slower at their own craft: **hunters** on the hunt, **foragers** on forage/verge, **scouts** on explore/survey/trail/track. only the long rest and daily stipend stay once-per-sunrise  
...
| `/weep`                                                 | silverrush only: release grief at the weep stone (unlimited; spends energy) |
...
✦ `/weep`; silverrush only: release grief alone at the weep stone (unlimited; spends energy; also eases the `grief_melancholy` condition)  
