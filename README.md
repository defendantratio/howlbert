...
✦ **few hard blocks; energy, not cooldowns**: instead of "come back next sunrise" walls, every field/social/den action spends **energy** (shown on `/vitals action:condition`). energy never refuses a command; acting at **0** just adds exhaustion and costs mood, on top of the exhaustion/pain-exhaustion layer rather than replacing it. energy restores each **sunrise** sleep, from `/vitals action:rest`, and via a slow **real-time drip** while a wolf is idle during the day, all scaled by **hunger** (a starving wolf recovers less). **hunting** is throttled purely by energy now (no daily cap; hunters spend far less energy per hunt, so they hunt many more times before tiring). separate from energy, **forage/verge** repeats still climb the dc in one sunrise (over-worked land yields less; full foragers exempt); only the long rest and daily stipend stay once-per-sunrise  
